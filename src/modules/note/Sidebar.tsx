...
import { useCreateQuickNote } from "./quickNote";
...
export const Sidebar: React.FC = () => {
  ...
  const {quickNote,handleCreateQuickNote} = useCreateQuickNote();
    ...
    <div className="flex flex-row bg-secondary">
      <div className="flex flex-col w-56">
        ...
        <ul className="flex flex-col px-2">
          ...
          <li>
            {quickNote._id ? (
              // If quick note exists, show link to quick note
              <Link href={`/quickNote/${quickNote._id}`} passHref>
                <a className="flex flex-row items-center h-10 rounded-lg text-muted-foreground hover:bg-primary/5">
                  <NoteAddIcon className="ml-2" fontSize="small" />
                  <span className="text-sm font-medium pl-2">Quick Note</span>
                </a>
              </Link>
            ) : (
              // If quick note doesn't exist, show button to create quick note
              <button
                className="flex flex-row w-full items-center h-10 rounded-lg text-muted-foreground hover:bg-primary/5"
                onClick={() => handleCreateQuickNote({ name: "Quick Note", document: "" })}
              >
                <AddCircleOutlineIcon className="ml-2" fontSize="small" />
                <span className="text-sm font-medium pl-2">Create Quick Note</span>
              </button>
            )}
          </li>
